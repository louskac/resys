  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-2 pl-3.5 border-l border-white/10 text-[10px] text-zinc-400 group-hover:text-zinc-200 transition-colors">
        {badgeText && (
          <span className="font-mono bg-white/[0.04] border border-white/[0.08] group-hover:border-tenant-primary/30 rounded px-1.5 py-0.5 select-none font-bold text-[9px] group-hover:text-tenant-primary transition-colors">
            {badgeText}
          </span>
        )}
        {showMicIcon && (
          <Mic size={12} className="text-tenant-primary shrink-0 animate-bounce drop-shadow-[0_0_4px_rgba(112,0,255,0.5)]" style={{ animationDuration: '2s' }} />
        )}
      </div>
    </button>
  );
